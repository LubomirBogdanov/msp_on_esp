			      ...
			      struct cmdline_args *args)
{
	...
	while ((opt = getopt_long(argc, argv, "d:jv:nU:s:qC:",
		...
		case LOPT_WIFI_BRIDGE_SKIP_INIT:
			args->devarg.flags |= DEVICE_WIFI_BRIDGE_SKIP_INIT;
			break;

		case LOPT_SOCKET_PORT:
			args->devarg.socket_port = (uint16_t) atoi(optarg);
			break;
...
int main(int argc, char **argv)
{
	...
	args.devarg.socket_port = 5556;
	memset(args.devarg.socket_ip, 0x00,
		   sizeof(args.devarg.socket_ip));
